...
public class PacketReader {
	...
	private String file;
	private TcpState flagState = new TcpState(TcpState.State.START); 
	...
	public BasicPacketInfo nextPacket(){
		 ...
		 try{
			 if(pcapReader.nextEx(hdr,buf) == Pcap.NEXT_EX_OK){
				 ...
				 //packet.scan(Ethernet.ID);	// critical MY 			 
		...
		try {
			...
			if (packet.hasHeader(ipv4)){
				...
				if(this.firstPacket == 0L)
					this.firstPacket = packet.getCaptureHeader().timestampInMicros();
				this.lastPacket = packet.getCaptureHeader().timestampInMicros();
	...
	private static BasicPacketInfo getIpv4Info(PcapPacket packet,Protocol protocol) {
		...
		try {
			...
			if (packet.hasHeader(protocol.getIpv4())){
				...
				/*if(packetInfo.firstPacket == 0L)
					packetInfo.firstPacket = packet.getCaptureHeader().timestampInMicros();
				packetInfo.lastPacket = packet.getCaptureHeader().timestampInMicros();*/
